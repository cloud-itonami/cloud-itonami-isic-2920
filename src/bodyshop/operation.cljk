(ns bodyshop.operation
  "OperationActor -- one body-in-white (BIW) stamping/welding
  body-shop operation = one supervised actor run, expressed as a
  langgraph-clj StateGraph. The advisor (Body Shop Advisor) is sealed
  into a single node (:advise); its proposal is ALWAYS routed through
  the Stamping Governor (:govern) and the rollout phase gate
  ...
  One graph run = one body-shop operation (intake -> advise -> govern
  ...
  decision to a human operator (the body-shop quality engineer). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/ship-body-shell`/`:actuation/issue-body-
  certificate` ALWAYS reach this node when the governor is clean --
  see `bodyshop.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [bodyshop.bodyshopadvisor :as bodyshopadvisor]
            [bodyshop.governor :as governor]
...
(defn build
  ...
  `bodyshop.store/Store`).
  opts:
    :advisor      -- a `bodyshop.bodyshopadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (bodyshopadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Body Shop Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (bodyshopadvisor/-advise advisor store request)]
            {:proposal p :audit [(bodyshopadvisor/trace request p)]})))

      ;; Stamping Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
